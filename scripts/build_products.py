#!/usr/bin/env python3
"""Build SEO-friendly static ValueRadar product pages and sitemap from products.json."""
...
def price_signal(s):
    ...
    context=s.get('price_context','marketplace_observed')
    label='Tracked reference price' if context=='third_party_tracker' else 'Marketplace price observed'
...
def page(p):
    ...
    verdict=e(p['verdict'].replace('_',' ')); canonical=f'https://valueradar.in/products/{e(p["slug"])}/'
    note=e(s.get('price_history_note') or 'Marketplace price, seller and availability can change. Verify the current listing before buying.')
...
def write_sitemap(products):
    urls=[f'https://valueradar.in/{x}' for x in STATIC_URLS]
    urls += [f'https://valueradar.in/products/{p["slug"]}/' for p in products if p.get('status')=='published']
    body='\n'.join(f'  <url><loc>{e(u)}</loc></url>' for u in urls)
...
def main():
    ...
    write_sitemap(db.get('products',[])); print(f'Built {len(built)} product page(s) and sitemap: '+(', '.join(built) if built else 'none'))
